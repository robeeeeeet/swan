...
- **最新コミット**: 5c348b4 - Tips評価システムとランキングページを実装

## 最新の機能追加（2025-12-07 NEW!）

### Tips評価システム
- **Good/Badボタン**: ダッシュボードでTipsをワンタップ評価（何度でも評価可能）
- **Wilson Score**: Reddit採用のアルゴリズムで公平なスコア計算
- **重み付け選択**: Good評価が多いTipsを優先表示（最低10%保証）
- **時間帯フィルタリング**: 朝・昼・夕・夜 + 平日・休日で適切なTips表示
- **Tips人気ランキング**: `/tips-ranking` でみんなの評価を確認可能
- **デュアルストレージ**: IndexedDB（ローカル）+ Firestore（クラウド）で評価を保存 ✅ NEW! (2025-12-07)
- **グローバルランキング**: 全ユーザーの評価を集計表示（「みんなの人気」タブ）
- **パーソナルランキング**: 自分の評価のみ表示（「あなたの評価」タブ）
- **カテゴリーフィルター改善**: 5列グリッドレイアウト（モバイル最適化）

### 履歴削除機能
- **「今日の目標」タップ** → 今日の履歴モーダル表示
- **「今日の記録」タップ** → 今日の履歴モーダル表示（「タップで詳細」ヒント付き）
- **履歴項目タップ** → 削除確認ダイアログ
- **削除実行** → IndexedDB + Firestore同期削除、カウンター即座更新

### 更新ボタン（ヘッダー）
- **ヘッダー左端に配置**: 回転矢印アイコン
- **機能**: Service Worker更新チェック + ページリロード
- **アニメーション**: 押下中はアイコンが回転
- **用途**: 新しいビルド反映時にアプリを再起動せずに更新可能
...
│   │   ├── settings.ts          ✅
│   │   ├── sync.ts              ✅
│   │   ├── tipRatings.ts        ✅ NEW! (2025-12-07) Tips評価ストレージ（IndexedDB）
│   │   └── index.ts             ✅
│   ├── firebase/
│   │   ├── ...
│   │   └── tipRatings.ts        ✅ NEW! (2025-12-07) Tips評価Firestoreストレージ
│   ├── utils/                   ✅ NEW!
│   │   ├── summary.ts           ✅
│   │   └── date.ts              ✅ NEW! (2025-12-01) タイムゾーン対応日付ユーティリティ
│   ├── tips/                    ✅ NEW! (2025-12-07)
│   │   ├── scoring.ts           ✅ Wilson Scoreアルゴリズム・重み付け選択
│   │   └── index.ts             ✅ エクスポート
...
│   ├── tags.ts                  ✅
│   ├── messages.ts              ✅
│   └── tips.ts                  ✅ 30種類カテゴリー別Tips（時間帯フィルタリング対応 2025-12-07）
├── public/
│   └── manifest.json            ✅
├── docs/
│   ├── requirements.md          ✅
...
2. **ランダムTips（B-02）**
   - 30種類のカテゴリー別Tipsから重み付けランダム表示
   - 時間帯・曜日でフィルタリング（例: シャワーは夜・休日のみ）
   - Good/Bad評価ボタン（Wilson Scoreで重み付け）
   - Tips人気ランキングへのリンク
   - 5分ごとに自動更新
