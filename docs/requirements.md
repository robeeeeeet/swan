...
- 30種類のカテゴリー別Tipsをプリセット（9カテゴリー）
- 時間帯・曜日フィルタリング（例: シャワーは夜・休日のみ）
- Good/Bad評価機能でパーソナライズ
- Wilson Score Lower Boundアルゴリズムで公平なランキング
- Tips人気ランキングページ（`/tips-ranking`）で評価確認

**受け入れ条件:** ✅ 実装完了（2025-12-07）
- [x] Tipsがランダムに表示される
- [x] 同じTipsが連続しない
- [x] Good/Bad評価ボタンで評価可能
- [x] 評価データはIndexedDB + Firestoreに保存
- [x] 「みんなの人気」「あなたの評価」の2モードで閲覧可能
...
**受け入れ条件:** ✅ 実装完了（2025-12-06）
- [x] 指定時間にPush通知が届く（毎朝7時JST - Cron）
- [x] 通知タップでアプリが開く
- [x] 昨日の実績に基づいたメッセージが表示される（Gemini AI生成）
...
**受け入れ条件:** ✅ 実装完了（2025-12-06）
- [x] 過去データから危険時間が予測される（getTypicalCravingHours）
- [x] 定期的に通知が届く（9:30, 12:30, 15:30, 18:30, 21:30 JST）
- [x] 通知内容がパーソナライズされている（Gemini AI生成）
...
**受け入れ条件:** ✅ 実装完了（2025-12-06）
- [x] 実績に基づいた目標が提案される（shouldSuggestStepDown判定）
- [x] 週1回通知（日曜20時JST）
- [x] Gemini AIによるパーソナライズメッセージ
...
**受け入れ条件:** ✅ 実装完了（2025-12-06）
- [x] 記録がない場合に通知が届く（4時間おき: 8, 12, 16, 20時 JST）
- [x] 優しいトーンのメッセージ（Gemini AI生成）
...
**受け入れ条件:** ✅ 実装完了（2025-12-06）
- [x] 状況に応じたメッセージが生成される（useSOSCoaching フック）
- [x] 数値が正確（ユーザーコンテキストから計算）
- [x] フォールバックメッセージ対応（AI障害時）
...
| バージョン | 日付 | 変更内容 |
|-----------|------|---------|
| 1.2.0 | 2025-12-07 | B-02 Tips評価システム追加（Wilson Score、デュアルストレージ、ランキング機能） |
| 1.1.0 | 2025-12-06 | C群・D-03・E-02の実装完了ステータス更新（Phase 3完了） |
